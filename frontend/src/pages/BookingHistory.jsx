...
export const BookingHistory = () => {
  ...
  const handleCancel = async (bookingId) => {
    if (
      !window.confirm(
        "Are you sure you want to cancel this booking? The slot will be freed and your crop will become available again.",
      )
    ) {
      return;
    }
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-4 mb-4">
                    <div>
                      <h2 className="text-xl font-semibold text-gray-800">
                        {booking.centre?.name || "Unknown Centre"}
                      </h2>
                    ...
                    <span
                      className={`px-3 py-1 mt-2 md:mt-0 rounded-full text-xs font-bold ${getStatusColor(booking.status)}`}
                    >
                      {booking.status}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-medium text-gray-900">
                        {booking.crop?.cropType || "Unknown"} -{" "}
                        {booking.crop?.quantity} quintals
                      </p>
                    </div>
                    <div>
